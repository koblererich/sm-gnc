...
\begin{abstract}
  In this paper, we propose a unified view of denoising diffusion-based models in the context of graded non-convex energy minimization. We show that for a sufficiently large diffusion parameter, the associated negative log prior becomes a convex function. As a consequence the denoising diffusion-based image models essentially implement a learned graded non-convexity heuristic. We show applications to image reconstruction (denoising and inpainting) by adopting the popular Fields-of-Experts image prior in our framework. Our results show that our method leads to better results with a significantly smaller number of iterations. Moreover, the schedule of the diffusion parameter can be set after learning the priors, which adds great flexibility to the model.
\end{abstract}
...
Further, denoising score-based generative models learn to approximate gradients of energies that become gradually more \emph{convex} for
increasing noise and hence recent inference techniques~\citep{SoMe22} essentially implement a graduated non-convexity~(GNC) principle~\citep{BlZi87}, which is a widely used heuristic for avoiding local minima.

%The basic idea of the GNC method is to gradually minimize a surrogate function which depends on a parameter $t\geq 0$ such that for $t \geq \tilde t$, the surrogate function becomes convex and in the limit $t \to 0$ the surrogate function is equivalent to the original function.

Inspired by these observations, we propose a unified energy-based
perspective of diffusion-based models that is based on learning a
one-parameter family of smoothed image priors, continuously parametrized by~$t$.
This enables us to choose a smoothing schedule after learning this family.
In particular, we compare the following options on typical inverse problems in imaging:
\begin{enumerate}
  ...
  \item A predefined schedule such that the subsequent inference
    becomes equivalent to the GNC principle.
  \item A task-specific learned schedule based on unrolling a certain
...
The key observation is that although the data becomes noisier with increasing~$t$, the corresponding probability density~$\pdf{\rvx\vert\rt}$ gets smoother (in fact more Gaussian) and its associated energy~$F(\vx,t)=-\log\pdf{\rvx\vert\rt}(\vx,t)$ gets more \emph{convex} (in fact more quadratic), see the left plot in Figure~\ref{fig:gmmOptimization}.
...
Thus, we focus in the following on probability density functions induced by a GMM.
...
\node[anchor=south] (r1) at (0,0) {\includegraphics[width=.85\linewidth]{./figures/results/params_foe}};
...
\node[anchor=south] (r2) at (0,-5.3) {\includegraphics[width=.85\linewidth]{./figures/results/params_foedeep}};
...
\node[anchor=north] (r3) at (0,-5.5) {\includegraphics[width=.85\linewidth]{./figures/results/params_foedeep3}};
...
\node[anchor=center] (x) at (0,0) {\includegraphics[width=.85\linewidth]{figures/results/foe_steps}};
% \draw[black]
...
% -----------------------------------------------------------------------------------------------------
\section{Conclusion}
In this work, we drew connections between diffusion-based models and the GNC principle by taking on the perspective of energies -- the negative log-density.
We observed that adding noise with increasing leads to smoother and more convex energies.
To account for this smoothing effect, we extended the FoE prior by using 2D activation functions and proposed learning the joint density of noisy-images and smoothing parameters using SM.
Using this framework, the learned priors can be easily incorporated in solving inverse problems, e.g., by MAP estimation.
Finally, the continuous differentiability w.r.t.~both arguments enabled an analysis of different smoothing schedule selections.
