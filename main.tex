...
\begin{abstract}
In this paper, we propose a unified view of denoising score-based models in the context of graduated non-convex energy minimization.
We show that for sufficiently large noise variance, the associated negative log density -- the energy -- becomes convex. Consequently, denoising score-based models essentially follow the graduated non-convexity principle.
In this framework, we learn extended Fields of Experts image priors to approximate the joint density of noisy images and variances in the negative log domain.
These priors can be easily incorporated into existing solution techniques for inverse problems and offer great flexibility in selecting variance sequences after training.
Our numerical experiments on image denoising and inpainting show that only a few steps are required during inference.
%   We show applications to image reconstruction (denoising and inpainting) by adopting the popular Fields-of-Experts image prior in our framework. Our results show that our method leads to better results with a significantly smaller number of iterations. Moreover, the schedule of the diffusion parameter can be set after learning the priors, which adds great flexibility to the model.
\end{abstract}
...
perspective of score-based models through learning a
...
% -----------------------------------------------------------------------------------------------------
\section{Conclusion}
In this work, we drew connections between diffusion-based models and the GNC principle by taking on the perspective of energies.
We observed that adding noise with increasing variance leads to smoother and more convex energies.
...
These priors can be easily incorporated into existing approaches for solving inverse problems such as MAP estimation.
