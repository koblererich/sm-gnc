...
The key observation is that although the data becomes noisier with increasing~$t$, the corresponding smooth probability density~$\pdf{\rvx\vert\rt}$ gets smoother and its associated energy~$F(\vx,t)=-\log\pdf{\rvx\vert\rt}(\vx,t)$ gets more \emph{convex}, see the top plot in Figure~\ref{fig:gmmOptimization}.
...
Bottom: If the smallest second derivative of~$F$ w.r.t.~$x$ is positive anywhere for~$t_0$ if $F$ it is convex.
}
\end{figure}
...
\end{align}
A streight forward approach, requiring little knowledge of the objective, is adapting an alternating proximal gradient scheme~\citep{BoSa14} along with Lipschitz backtracking of~\citet{BeTe09}.
However, we instead exploit the fact that~$\nabla_\vx R_L(\cdot,\hatt)$ is approximatelly $\exp(-\hatt)$-Lipschitz and propose the preconditioned proximal gradient algorithm listed in \Algref{alg:jointMinimization}.
...
In fact it is equivalent to the GNC flow presented in~\Algref{alg:graduatedNC}.
...
To do so, we propose to ``unroll'' the optimization scheme~\eqref{eq:fixedScheme} for $I\in\N$ steps and learn all~$\{\hatt_i\}_{i=1}^I$ and individual step sizes~$\{\eta_i\}_{i=1}^I$ such that the final output~$\vx_n$ is close to its corresponding ground truth.
...
Thus, VNs -- with temporally changing parameters across the steps -- can be interpreted as a learned proximal-gradient scheme on a gradually more non-convex energy.
...
Moreover, if only the smoothing schedule and the step sizes are learned, just a few paired training samples are required for supervized learning due to the small parameter space.
...
\node[anchor=south] (r2) at (0,-6.25) {\includegraphics[width=.96\linewidth]{./figures/results/params_foedeep}};
...
\node[anchor=north] (r3) at (0,-6.75) {\includegraphics[width=.96\linewidth]{./figures/results/params_foedeep3}};
